    ...
    <div className=' min-h-screen flex justify-center items-center'>
        ...
        <form className=' bg-background text-primary w-full max-w-sm p-7 border rounded-2xl flex flex-col gap-5 mx-5' onSubmit={LoginSubmit(onLogin,onError)}>
            ...
            <div className=' flex flex-col gap-2'>
                ...
                <input id="login-email" type="email" placeholder='e.g. jhon@gmail.com' className=' w-full p-2 rounded-lg border outline-0' {...LoginForm('email')} />
            </div>
            <div className=' flex flex-col gap-2'>
                <label htmlFor="login-password" className=' text-sm text-muted-foreground'>Password</label>
                <input id="login-password" {...LoginForm('password')} type="password" placeholder='e.g. Jhon123' className=' w-full p-2 rounded-lg border outline-0' />
         ...
         {/* Forget Email  */}
         {
            ...
            <div className=' flex flex-col gap-2'>
                <label htmlFor="forget-email" className=' text-sm text-muted-foreground'>Email</label>
                <input id="forget-email" type="email" placeholder='e.g. jhon@gmail.com' className=' w-full p-2 rounded-lg border outline-0' {...forgetEmail.register('email')}/>
            </div>
         ...
         {forgetOtpModel && <form onSubmit={otpVerify.handleSubmit(otpVerifySubmit,onError)} className=' bg-background text-primary w-full max-w-sm p-7 border rounded-2xl flex flex-col gap-5 mx-5'>
            ...
            <div className=' flex flex-col gap-2'>
                ...
                <input id="otp" type="text" placeholder='e.g. 123456' className=' w-full p-2 rounded-lg border outline-0' {...otpVerify.register("otp")}/>
            </div>
        ...
        { forgetPasswordModel && <form className=' bg-background text-primary w-full max-w-sm p-7 border rounded-2xl flex flex-col gap-5 mx-5' onSubmit={resetPassword.handleSubmit(resetPasswordSubmit,onError)}>
            <h2 className=' flex items-center gap-1 hover:text-indigo-500 cursor-pointer' onClick={()=>{setForgetPasswordModel(false); setEmail('')}}><ArrowLeft className='  cursor-pointer size-5'/>Back</h2>
            ...
            <div className=' flex flex-col gap-2'>
                ...
                <input id="new-password" type="password" placeholder='e.g. Jhon123' className=' w-full p-2 rounded-lg border outline-0' {...resetPassword.register('password')}/>
            </div>
            <div className=' flex flex-col gap-2'>
                <label htmlFor="confirm-password" className=' text-sm text-muted-foreground'>Confirm Password</label>
                <input id="confirm-password" type="password" placeholder='e.g. Jhon123' className=' w-full p-2 rounded-lg border outline-0' {...resetPassword.register('confirmPassword')}/>
            </div>
